...
const feature = loadFeature("features/security-add-practitionerrole.feature")
  ...
  test("Add a new PractitionerRole", ({ given, when, then }) => {
    ...
    given("A Location hierarchy exists", searchResults => {
      ...
      axios.__setFhirResults( DEFAULT_URL + "Location",
        { _id: "Location/facility", status: "active", "_include:iterate": "Location:partof" }, results )
    } )
    when("A new PractitionerRole is submitted", role => {
