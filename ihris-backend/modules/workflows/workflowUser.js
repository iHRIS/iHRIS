...
const sendEmail = ihrissmartrequire("sendEmail");
    ...
    process: (req) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let userInput = req.body.item[0].item;
                let userEmail = userInput.find(x => x.linkId === "Person.telecom[0].value")?.answer[0]?.valueString?.toLowerCase();
                let statusInput = userInput.find(x => x.linkId === "Person.active")?.answer[0]?.valueString
                let role = userInput.find(x => x.linkId === "role")?.answer[0]?.valueReference
                let location;

                if ( req.body && req.body.item
                  && req.body.item
                  && req.body.item[0].item[5].linkId === "location"
                  && req.body.item[0].item[5].answer
                  && req.body.item[0].item[5].answer[0]
                  && req.body.item[0].item[5].answer[0].valueReference){
                location = userInput.find(x => x.linkId === "location")?.answer[0]?.valueReference
                }
                ...
                let userId = uuidv4()
                user.lookupByEmail(userEmail,false)
                    .then(async (userObj) => {
                        if (!userObj) {
                            if (location) {
                                try {
                                    userRoles = await fhirAxios.search("Basic", {
                                        locationconstraint: "related-location=" + location,
                                    });
                                } catch (err) {
                                    winston.error("Error Setting User roles for User" + req.body.item[0].item[3].answer[0].valueString);
                                    resolve(await workflowUser.outcome("Error Setting User roles for User" + req.body.item[0].item[3].answer[0].valueString));
                                }
                                let locationValueReference = location.reference;
                                if (userRoles.entry) {
                                    ...
                                    try {
                                        ...
                                        resolve(await workflowUser.outcome("TESTING NEW LOCATION "));
                                        // extensions.push({
                                        //     url: ROLE_EXTENSION,
                                        //     valueReference: { reference: "Basic/" + userRoleId},
                                        // });
                            ...
                            if (practitioner && practitioner.answer[0].valueReference.reference != null) {
                                extensions.push({
                            ...
                            let name = "";
                            let userNameInput = req.body.item[0].item.find(x => x.linkId === "Person.name[0].text").answer[0].valueString
                            if (userNameInput) {
                                name = userNameInput
                            } else {
                                resolve(await workflowUser.outcome("No Name Provided for this User"));
                            }
                            if (statusInput) {
                                status = statusInput
                            }
                ...
                console.log("ERROR IN MAIN TRY :", err)
                winston.error(err);
                reject(err.message);
            }
        });
