    ...
    fhirAxios.read("Basic", page).then(async (resource) => {
        ...
        const createTemplate = async (resource, structure) => {
            ...
            for (let fhir of structureKeys) {
                ...
                vueOutput = '<' + resourceElement + ' :fhir-id="fhirId" :fhir-version="fhirVersion" :show-drawer="showDrawer" :edit="isEdit" v-on:set-edit="setEdit($event)" profile="' + resource.url + '" :key="$route.params.page+($route.params.id || \'\')" page="' + req.params.page + '" field="' + fhir + '" title="' + sections[fhir].title + '" :constraints="constraints"'
