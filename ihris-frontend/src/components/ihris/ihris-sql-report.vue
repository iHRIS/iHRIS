...
export default {
  ...
  methods: {
    ...
    buildTerms() {
      ...
      if (Object.keys(this.terms).length > 0) {
        for (let sTerm in this.terms) {
          let dataType = this.reportData.filters && this.reportData.filters.find((filter) => {
          ...
          if(dataType === 'date' && !sTermDet.isDropDown) {
            let date = this.terms
            let filterDate = []
    ...
    getTotalRecords() {
      let url = `/fhir2sql/reportData/${this.reportData.indexName}/count`;
    ...
    getData(restart) {
      ...
      url = `/fhir2sql/reportData/${this.reportData.indexName}`;
